defmodule CircleciCli.HTTP.BuilderTest do
  ...
  test "user command" do
    assert_build ["user"], [url: "https://circleci.com/api/v1/me?circle-token=#{@token}", method: :get]
  end

  test "projects command" do
     assert_build ["projects"], [url: "https://circleci.com/api/v1/projects?circle-token=#{@token}", method: :get]
  end

  test "builds command" do
     assert_build ["builds"], [url: "https://circleci.com/api/v1/recent-builds?circle-token=#{@token}", method: :get]
  end

  test "user-key command" do
    assert_build ["user-key"], [url: "https://circleci.com/api/v1/user/ssh-key?circle-token=#{@token}", method: :post]
  end

  test "clear-cache command" do
    result = [url: "https://circleci.com/api/v1/project/#{@user}/#{@project}/build-cache?circle-token=#{@token}", method: :delete]
  ...
  test "build command" do
    result = [url: "https://circleci.com/api/v1/project/#{@user}/#{@project}/#{@build}?circle-token=#{@token}", method: :get]
  ...
  test "artifacts command" do
    result = [url: "https://circleci.com/api/v1/project/#{@user}/#{@project}/#{@build}/artifacts?circle-token=#{@token}", method: :get]
  ...
  test "retry command" do
    result = [url: "https://circleci.com/api/v1/project/#{@user}/#{@project}/#{@build}/retry?circle-token=#{@token}", method: :post]
  ...
  test "cancel command" do
    result = [url: "https://circleci.com/api/v1/project/#{@user}/#{@project}/#{@build}/cancel?circle-token=#{@token}", method: :post]
  ...
  test "trigger command" do
    result = [url: "https://circleci.com/api/v1/project/#{@user}/#{@project}/tree/master?circle-token=#{@token}", method: :post]
  ...
  test "project-key command" do
    result = [url: "https://circleci.com/api/v1/project/#{@user}/#{@project}/ssh-key?circle-token=#{@token}", method: :post]
    assert_build ["project-key", "--user=#{@user}", "--project=#{@project}"], result
    assert_build ["project-key", "-u", @user, "-p", @project],                result
  end

  test "heroku-key command" do
    result = [url: "https://circleci.com/api/v1/user/heroku-key?circle-token=#{@token}", method: :post, body: "{apikey: #{@key}}"]
